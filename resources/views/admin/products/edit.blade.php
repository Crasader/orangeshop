                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body no-padding">
                        ...
                        <div class="nav-tabs-custom">
                            ...
                            <form action="{{ route('product.update',['pid'=>$product->pid]) }}" class="form-horizontal" method="post"
                                  id="product_form">
                                <div class="tab-content">
                                    ...
                                    <div class="tab-pane active" id="tab_1">
                                            ...
                                            <div class="col-sm-8">
                                                <select class="form-control select2" style="width: 100%;"
                                                    ...
                                                    @foreach($brands as $brand)
                                                        <option value="{{ $brand->brand_id }}" data-cates="{{ $brand->categories->pluck('cate_id')}}"
                                                                @if($product->brand_id == $brand->brand_id) selected @endif>{{ $brand->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-8">
                                                ...
                                                @foreach($brands->where('brand_id',$product->brand_id)->first()->categories as $cate)
                                                        <option value="{{ $cate->cate_id }}"
                                                                @if($cate->cate_id == $product->cate_id) selected @endif>
                                                            {{ $cate->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-8">
                                                <textarea name="keywords" id="" cols="30" rows="5"
                                                          style="width: 100%">{{$product->keywords }}</textarea>
                                            </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-8">
                                                <input type="text" class="form-control" placeholder="排序"
                                                       name="order" value="{{ $product->order }}">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-8">
                                                <label>
                                                    <input type="radio" class="form-control"
                                                           name="state" value="1"
                                                           @if($product->state == 1) checked @endif>
                                                    开启
                                                </label>
                                                <label>
                                                    <input type="radio" class="form-control"
                                                           name="state" value="0"
                                                           @if($product->state == 0) checked @endif>
                                                    关闭
                                                </label>
                                            </div>
                                        </div>
                                                    ...
                                                    <div class="attr_values">
                                                        @if($attr->show_type == 2)
                                                            @foreach($attr->attribute_values as $value)
                                                                ...
                                                                <input type="text" class="add-money" placeholder="附加值"
                                                                       value="{{ $product->attribute_values->where('attr_value_id',$value->attr_value_id)->first()?$product->attribute_values->where('attr_value_id',$value->attr_value_id)->first()->pivot->add_money:''}}">
                                                            @endforeach
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-8">
                                                <input type="text" class="form-control" placeholder="预警"
                                                       name="stock_limit" value="{{ $product->stock_limit }}">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                    ...
                                    <div class="tab-pane" id="tab_4">
                                        <textarea name="description" id="description" cols="30" rows="10"
                                                  style="height: 400px;max-height: 600px">{{ $product->description }}</textarea>
                                    </div>
                                    <!-- /.tab-pane -->
...
@section('script')
    ...
    <script>
        $(document).ready(function () {
            ...
            //money不为空的属性值应该选中,并且处理checkbox的value值
            var dom_money = $('.add-money');
            for(var i =0; i<dom_money.size(); i++){
                var money = dom_money.eq(i).val();
                if(money!= ''){
                    target = dom_money.eq(i).prev().find('input[type="checkbox"]');
                    target.iCheck('check');
                    target.val(target.data('id') + "_" + money);
                }
            }
            //处理添加额外钱事件
