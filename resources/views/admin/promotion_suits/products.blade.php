                    ...
                    <table class="table table-bordered table-stripped" v-cloak>
                        ...
                        <tr v-for="(product,index) in products_table">
                            ...
                            <td>
                                ...
                                product.pid" data-pm_id = {{ $promotion_suit->pm_id }}>
                    ...
                    <table class="table table-bordered table-stripped" v-cloak=>
                        ...
                        <tr v-for="(product,index) in products">
                            ...
                            <td>
                                ...
                                "{{ $promotion_suit->pm_id }}" :data-pid="product.pid">
    ...
    <script>
        $(function () {
            ...
            var vue = new Vue({
                ...
                data: {
                    products: [],
                    products_table: {!! $promotion_suit->products !!}
                },
                methods: {
                    add_product: function (index, event) {
                        ...
                        var discount = btn.closest('tr').find('input').eq(0).val();
                        console.log(discount);
                        var number = btn.closest('tr').find('input').eq(1).val();
                        ...
                        //添加商品
                        $.ajax({
                            url: '/admin/promotion_suit/' + pm_id + '/add_product',
                            type: 'post',
                            dataType: 'json',
                            data: {
                            ...
                            success: function (data) {
                                if (data.error == 0) {
                                    toastr.success('添加成功');
                                    //数据添加到 table中
                                    var pivot = {"discount": discount, "number": number};
                                    self.products[index].pivot = pivot;
                    ...
                    remove_product: function (index, event) {
                        ...
                        //发送ajax进行移除商品
                        $.ajax({
                            url: '/admin/promotion_suit/' + pm_id + '/remove_product',
                            type: 'delete',
                            dataType: 'json',
                            data: {
                                _token: $('meta[name="_token"]').attr('content'),
            ...
            //提交表单
            $('#promotion_suit').submit(function () {
                if ($('#pid').val() == "") {
                    swal({
                        title: '没有选择商品...',
                        type: 'warning'
