                ...
                <div class="col-xs-6">
                    <table class="table table-bordered table-stripped" v-cloak>
                        ...
                        <tr v-for="(product,index) in products_table">
                            ...
                            <td >@{{ product.pivot.type == 1?'主品':'赠品' }}</td>
                            <td>
                                <button class="btn btn-xs btn-danger" @click='remove_product(index,$event)' :data-pid="
                                product.pid" data-pm_id = {{ $promotion_full_send->pm_id }}>
                        ...
                        <tr v-for="(product,index) in products">
                            ...
                            <td>
                                <div class="btn-group btn-group-xs">
                                    <button class="btn btn-warning" @click="add_product(index,$event)" data-pm_id=
                                    "{{ $promotion_full_send->pm_id }}" :data-pid="product.pid" data-type="1">
                                    <i class="fa fa-plus"> 主品</i>
                                    </button>
                                    <button class="btn btn-primary" @click="add_product(index,$event)" data-pm_id=
                                    "{{ $promotion_full_send->pm_id }}" :data-pid="product.pid" data-type="0">
                                    <i class="fa fa-plus"> 赠品</i>
                                    </button>
                                </div>
                            </td>
                        </tr>
    ...
    <script>
        $(function () {
            ...
            var vue = new Vue({
                ...
                data: {
                    products: [],
                    products_table: {!! $promotion_full_send->products !!}
                },
                methods: {
                    add_product: function (index, event) {
                        ...
                        //添加商品
                        $.ajax({
                            url: '/admin/promotion_full_send/' + pm_id + '/add_product',
                            type: 'post',
                            dataType: 'json',
                            data: {
                    ...
                    remove_product: function (index, event) {
                        ...
                        //发送ajax进行移除商品
                        $.ajax({
                            url: '/admin/promotion_full_send/' + pm_id + '/remove_product',
                            type: 'delete',
                            dataType: 'json',
                            data: {
                                _token: $('meta[name="_token"]').attr('content'),
            ...
            //提交表单
            $('#promotion_full_send').submit(function () {
                if ($('#pid').val() == "") {
                    swal({
                        title: '没有选择商品...',
                        type: 'warning'
